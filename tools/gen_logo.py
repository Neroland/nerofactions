#!/usr/bin/env python3
"""
Generate the NEROFACTIONS mod logo (square), in the shared Neroland family style
...
the reputation/factions mod, so the core-prism is mounted on a heraldic CRIMSON shield with crossed banners Renders supersampled, then downsamples.

Outputs:
  art/logo/nerofactions_logo.png       (1024x1024 master)
  art/logo/nerofactions_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/nerofactions_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 37
MOD = 'nerofactions'
NAME = 'NEROFACTIONS'
ACCENT = (214, 66, 74)
ACCENT_BRIGHT = (255, 132, 138)
BRIGHT = (255, 230, 230)
NEBULA = (118, 26, 32)
STAR_TINT = (255, 200, 200)
NAME_GLOW = (214, 66, 74, 255)
...
TAG = 'F A C T I O N S   ·   R E P U T A T I O N   ·   T E R R I T O R Y'

def emblem_frame(base, cx, cy, rad):
    # crossed banner poles behind the shield
    bl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); bd = ImageDraw.Draw(bl)
    for sgn in (-1, 1):
        bd.line([cx + sgn * rad * 1.7, cy - rad * 1.7, cx - sgn * rad * 1.3, cy + rad * 2.0],
                fill=STARSTEEL + (210,), width=SS * 4)
        px = cx + sgn * rad * 1.7; py = cy - rad * 1.7
        bd.polygon([(px, py), (px + sgn * rad * 0.7, py + rad * 0.16),
                    (px, py + rad * 0.42)], fill=ACCENT + (235,))
    base.alpha_composite(bl)
    # heraldic shield medallion
    ...
    # chevron accent low on the shield
    gd.line([(cx - w * 0.7, cy + rad * 0.7), (cx, cy + rad * 1.25),
             (cx + w * 0.7, cy + rad * 0.7)], fill=ACCENT + (230,), width=SS * 3, joint="curve")
